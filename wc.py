...
class Search():
    def __init__(self, field, mode = "astar") -> None:
        self.itemcount = 0 # unique id for fields generated by A*
    ...
    def DFS(self, field, sum, moves, states):
        ...
        together with list of all the states on the path. 
        '''
        # BASE CASE:
        # Game is solved
        if sum == 0:
        ...
        # Iterate over field, starting from top left:
        for y in range(0,width):
            for x in range(0,height):
                # Check if cell is not empty:
                if field[x,y] > 0:
                    # Check if cell has neighbours and island of >= 1 neighbours: "poppable"
                    nb = self.CheckNeighbours(field,visited,field[x,y],x,y)
                    if len(nb) > 1: 
                        # Generate new field with popped bubbles:
                        newfield = self.UpdateField(field,nb)
                        # Call method recursively with new field:
    ...
    def GenerateChildrenAstar(self, field, moves, states, q, visitedStates):
        '''
        Generates all possible children of current node (field) and puts them in priority queue q when field configuration has not been in q yet.
        Returns q (priority queue) and visitedStates (dict).
        '''
        height = field.shape[0]
        width = field.shape[1]
            ...
            for x in range(0,height):
                if field[x,y] > 0:
                    ...
                    if len(nb) > 1: # Cell has island of >= 2 cells
                        ...
                        newfield = self.UpdateField(field,nb)
                        # Check if child (tuple form) has not been visited:
    ...
    def Astar(self, field, moves, states, q):# iterative
        ...
        while not q.empty():
            ...
            # Generate children of current node and update queue and visitedStates:
